 ...
 * @brief Defines angle and bond kernels on CUDA-supporting GPUs
 * @date 2021-11-29
 * 
 */
...
// Partice list
__constant__ MxParticleCUDA *cuda_bonds_parts;
...
static MxParticleCUDA *cuda_bonds_parts_arr;
...
// Angle arrays
__device__ MxAngleCUDA *cuda_angles;
static MxAngleCUDA *cuda_angles_device_arr;
static struct MxAngleCUDAData *cuda_angles_angles_arr, *cuda_angles_angles_arr1, *cuda_angles_angles_arr2;
static struct MxAngleCUDAData *cuda_angles_angles_local, *cuda_angles_angles_local1, *cuda_angles_angles_local2;
    ...
    if(using_engine) { 
        ...
        for(i = 0; i < e->nr_devices; i++) 
            if(e->devices[i] == cuda_bonds_device) { 
                cuda_bonds_parts_arr = (MxParticleCUDA*)e->parts_cuda[i];
        ...
        MxParticleCUDA *bonds_parts = (MxParticleCUDA*)malloc(e->s.nr_parts * sizeof(MxParticleCUDA));
        for(i = 0; i < e->s.nr_parts; i++) 
            bonds_parts[i] = MxParticleCUDA(e->s.partlist[i]);
        if(cudaMalloc(&cuda_bonds_parts_arr, e->s.nr_parts * sizeof(MxParticleCUDA)) != cudaSuccess) 
            return cuda_error(engine_err_cuda);
        if(cudaMemcpy(cuda_bonds_parts_arr, bonds_parts, e->s.nr_parts * sizeof(MxParticleCUDA), cudaMemcpyHostToDevice) != cudaSuccess) 
            return cuda_error(engine_err_cuda);
        if(cudaMemcpyToSymbol(cuda_bonds_parts, &cuda_bonds_parts_arr, sizeof(void*), 0, cudaMemcpyHostToDevice) != cudaSuccess) 
            return cuda_error(engine_err_cuda);
        free(bonds_parts);
...
__host__ 
MxAngleCUDA::MxAngleCUDA() {
    this->flags = ~ANGLE_ACTIVE;
}

__host__ 
MxAngleCUDA::MxAngleCUDA(MxAngle *a) : 
    flags{a->flags}, 
    dissociation_energy{(float)a->dissociation_energy}, 
    half_life{(float)a->half_life}, 
    pids{a->i, a->j, a->k}
{
    this->p = MxToCUDADevice(*a->potential);
}

__host__ 
void MxAngleCUDA::finalize() {
...
struct MxAngleCUDAData {
    uint32_t id;
    int3 cid;

    MxAngleCUDAData(MxAngle *a, uint32_t aid) : 
        id{aid}, 
        cid{
...
int cuda_angles_arrays_malloc() {
    size_t size_angles = cuda_bonds_nrparts_chunk * sizeof(MxAngleCUDAData);
    ...
    if(cudaMalloc(&cuda_angles_angles_arr2, size_angles) != cudaSuccess) 
        return cuda_error(engine_err_cuda);
    if((cuda_angles_angles_local1 = (MxAngleCUDAData*)malloc(size_angles)) == NULL) 
        return engine_err_malloc;
    if((cuda_angles_angles_local2 = (MxAngleCUDAData*)malloc(size_angles)) == NULL) 
        return engine_err_malloc;
...
int cuda_angles_angles_initialize(engine *e, MxAngle *angles, int N) {
    size_t size_angles = N * sizeof(MxAngleCUDA);
...
int cuda_angles_angles_finalize(engine *e) {
    size_t size_angles = cuda_angles_size * sizeof(MxAngleCUDA);
...
int cuda_angles_angles_extend() {
    ...
    MxAngleCUDA *cuda_angles_device_arr_new;
    if(cudaMalloc(&cuda_angles_device_arr_new, Nf * sizeof(MxAngleCUDA)) != cudaSuccess) 
        return cuda_error(engine_err_cuda);
    
    engine_cuda_memcpy<MxAngleCUDA><<<nr_blocks, nr_threads>>>(cuda_angles_device_arr_new, cuda_angles_device_arr, Ni * sizeof(MxAngleCUDA));
    if(cudaPeekAtLastError() != cudaSuccess) 
        return cuda_error(engine_err_cuda);

    if(cudaFree(cuda_angles_device_arr) != cudaSuccess) 
        return cuda_error(engine_err_cuda);
    if(cudaMalloc(&cuda_angles_device_arr, Nf * sizeof(MxAngleCUDA)) != cudaSuccess) 
        return cuda_error(engine_err_cuda);

    engine_cuda_memcpy<MxAngleCUDA><<<nr_blocks, nr_threads>>>(cuda_angles_device_arr, cuda_angles_device_arr_new, Ni * sizeof(MxAngleCUDA));
    if(cudaPeekAtLastError() != cudaSuccess) 
        return cuda_error(engine_err_cuda);
...
int engine_angle_cuda_initialize(engine *e) {
    ...
    if(cuda_angles_nr_blocks == 0) {
        cuda_angles_nr_blocks = MxCUDA::maxBlockDimX(0);
    }

    if(cuda_angles_nr_threads == 0) {
        cuda_angles_nr_threads = MxCUDA::maxThreadsPerBlock(0);
    }
...
__global__ 
void engine_cuda_set_angle_device(MxAngleCUDA a, unsigned int aid) {
    if(threadIdx.x > 0 || blockIdx.x > 0) 
        return;
...
int engine_cuda_finalize_angle(int aind) {
    ...
    MxAngleCUDA *angles_cuda = (MxAngleCUDA*)malloc(cuda_angles_size * sizeof(MxAngleCUDA));
    if(cudaMemcpy(angles_cuda, cuda_angles_device_arr, size_angles, cudaMemcpyDeviceToHost) != cudaSuccess) 
        return cuda_error(engine_err_cuda);
...
int engine_cuda_refresh_angle(MxAngleHandle *ah) {
    if(engine_cuda_finalize_angle(ah->id) < 0) 
        return error(engine_err);

    if(engine_cuda_add_angle(ah) < 0) 
        return error(engine_err);
...
__global__ 
void engine_cuda_set_angles_device(MxAngleCUDA *angles, unsigned int *aids, int nr_angles) {
    int tid = threadIdx.x + blockIdx.x * blockDim.x;
    int stride = gridDim.x * blockDim.x;
...
int engine_cuda_add_angles(MxAngleHandle *angles, int nr_angles) {
    MxAngleHandle *a;
    uint32_t aidmax = 0;

    MxAngleCUDA *acs = (MxAngleCUDA*)malloc(nr_angles * sizeof(MxAngleCUDA));
    MxAngleCUDA *acs_d;
    if(cudaMalloc(&acs_d, nr_angles * sizeof(MxAngleCUDA)) != cudaSuccess) 
        return cuda_error(engine_err_cuda);
    ...
    if(cudaMemcpyAsync(acs_d , acs , nr_angles * sizeof(MxAngleCUDA) , cudaMemcpyHostToDevice, *cuda_bonds_stream) != cudaSuccess) 
        return cuda_error(engine_err_cuda);
...
int engine_cuda_refresh_angles(engine *e, MxAngleHandle *angles, int nr_angles) { 
    int *ainds = (int*)malloc(nr_angles * sizeof(int));

    for(int i = 0; i < nr_angles; i++) 
        ainds[i] = angles[i].id;

    if(engine_cuda_finalize_angles(e, ainds, nr_angles) < 0) 
        return error(engine_err);
...
__global__ 
void angle_eval_cuda(MxAngleCUDAData *angles, int nr_angles, float *forces, float *epot_out, bool *toDestroy) {
    int tid = threadIdx.x + blockDim.x * blockIdx.x;
    int stride = blockDim.x * gridDim.x;
    int i, k;
    MxAngleCUDA *a;
...
int engine_angle_cuda_load_angle_chunk(MxAngle *angles, int loc, int N) { 
    size_t size_angles = N * sizeof(MxAngleCUDAData);
    size_t size_potenergies = N * sizeof(float);
    size_t size_forces = 6 * size_potenergies;
    size_t size_todestroy = N * sizeof(bool);
    MxAngle *buff = &angles[loc];
...
int engine_angle_cuda_unload_angle_chunk(MxAngle *angles, int loc, int N, struct engine *e, double *epot_out) { 
    int i, k;
    float epot = 0.f;
    float *bufff, ee, fi, fk;
    MxAngle *buffa = &angles[loc];
...
int MxAngleCUDA_refreshAngles(engine *e, MxAngleHandle **angles, int nr_angles) {
    ...
    MxAngleHandle *as = (MxAngleHandle*)malloc(nr_angles * sizeof(MxAngleHandle));
    MxAngleHandle *ah;
    for(int i = 0; i < nr_angles; i++) { 
        ah = angles[i];
        if(ah == NULL) 
